...
class DataQualityChecker:
    ...
    def _check_date_coverage(self) -> Dict[str, Any]:
                        ...
                        '$setToArray': {
                            '$map': {
                                'input': {
                                    '$range': [0, {
                                        '$dateDiff': {
                                            'startDate': {'$dateFromString': {'dateString': '$dh_utc'}},
                                            'endDate': {'$dateFromString': {'dateString': '$dh_utc'}},
                                            'unit': 'day'
                                        }
                                    }]
                                },
        ...
        try:
            date_result = list(self.db[COLLECTION_OBSERVATIONS].aggregate(date_pipeline))
            if date_result:
                result = {
                ...
                result = {'error': 'No date data'}
        except Exception as e:
            logger.warning(f'Date coverage check error: {e}')
            result = {'error': str(e)}
    ...
    def generate_report(quality_report: Dict) -> str:
        ...
        if 'date_coverage' in checks:
            ...
            if 'error' not in coverage:
                lines.append(f'    Min date: {coverage.get("min_date", "N/A")}')
                lines.append(f'    Max date: {coverage.get("max_date", "N/A")}')
            else:
